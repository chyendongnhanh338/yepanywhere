...
`
	...
	if got[0].ID != "emulator-5554" || got[0].Type != "emulator" || got[0].AVD != "Pixel_7_API_34" || got[0].State != "running" {
		t.Fatalf("unexpected emulator entry: %+v", got[0])
	}
	if got[1].ID != "R3CN90ABCDE" || got[1].Type != "android" || got[1].AVD != "R3CN90ABCDE" || got[1].State != "running" {
